                            ...
                            <div class="left-area h-100">
                                <div class="product_left">
                                    <div class="stall_subdiv mt-5">
                                        <img  :src="imageUrl+singleProduct.image[0].image" alt="image.image"  class="img-fluid productsingle_image">
                                    </div>
                                </div>
                                <div class="multiple_image">
                                    <div class="img_one">
                                        <carousel :autoplay="false" :navigation-enabled="true"  :data="data"  :controls="true" :dots="true" :nav="false" :loop="false" :responsive="{600:{items:2},900:{items:4},1200:{items:4}}">
                                            <!-- :autoplaySpeed="1000" -->
                                             <img v-for="(image, index) in singleProduct.image" :key="index" :src="imageUrl+image.image" alt="image.image"  @click="changeImage(image.image)" class="img-fluid multiple_image_one">
                                        </carousel>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-md-7 h-100">
                            <div class="product_right pt-5  h-100">
                                <div class="row bg-light p-5 product-content-area">
                                    <h2 class="text-success">{{singleProduct.product_name}}</h2>
                                    <table class="product-properties-table mt-3">
                                        ...
                                        <tr>
                                            <th>Category</th>
                                            <td>: {{singleProduct.category.category_name}}</td>
                                        </tr>
                                        <tr>
                                            <th>Sub Category</th>
                                            <td>: {{singleProduct.subCategory.sub_category_name}}</td>
                                        </tr>
                                    </table>
                                    <h6 class="mt-3 mb-3">
...
<!--                                    <div class="action-area mt-5" v-if="user.user.role === 'Customer'">-->
                                    <div class="action-area mt-5">
                                        <button @click="addToCart" class="mr-3 btn_one">Add to Cart</button>
                                        <a href="" class="btn_two">Chat With Seller</a>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    methods:{
        getSingleProduct(){
            axios.get(this.apiurl+'/api/products/'+this.$route.params.id).then(res=>{
                ...
                this.singleImage=this.singleProduct.image[0].image
        ...
        addToCart(){
            axios.post(this.apiurl+'/api/cart', {stall_id:this.singleProduct.stall_id, product_id:this.singleProduct.id}).then(res=>{
                 this.getCartItems()
            })

            // axios.get(this.apiUrl+'/api/cart/').then(response=>{
            //     //   this.cartCounter=res.data
            //     // let cartArea = this.$el.querySelector('#cartCount')
            //     //  cartArea.html(res.data)
